...
while true; do
  ...
  if [ ${#PODS[@]} -ne 0 ]; then
    echo "found pods : " $(echo $PODS) # DEBUG
    for p in "${PODS[@]}"; do
      pod_array=$(get_pod_array "${p}")
      # only schedule our pods
      if [ "$(eval echo \${$pod_array[1]})" == "${SCHEDULER}" ]; then
	      echo "scheduling " "$pod_array[1]" # DEBUG
        ...
        echo "Got the return code" # DEBUG
